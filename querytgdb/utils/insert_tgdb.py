...
def insert_metadata_analysis(metadict):
    try:
    ...
    # if experiment ID already exists: check if for the same exp, analysis_id provided in the current file also
    # exists
    if metadict['Experiment_ID'.upper()].upper().strip() in allmetaid:
        ...
        ref_data = ReferenceId.objects.filter(meta_id__meta_fullid__exact=metadict['Experiment_ID'.upper()].upper(),
                                              analysis_id__analysis_fullid__exact=metadict[
                                                  'Analysis_ID'.upper()].upper()) \
            .values('analysis_id')

        if not ref_data:
            exist_mdata_tmp = ReferenceId.objects.filter(
                meta_id__meta_fullid__exact=metadict['Experiment_ID'.upper()].
    ...
    if meta_exist_flag == 0:
        ...
        for i in metadict:
            ...
            if not i.upper() in analysis_datatypes:
                ...
                analysisdata_obj = AnalysisIddata(analysis_value=metadict[i], analysis_type=i,
                                                  analysis_id=adata_obj)
...
def insert_nodes_target(datalist, metadict, curr_metaid, curr_analysisid, curr_refid):
...
def insert_nodes_binding(datalist, metadict, curr_metaid, curr_analysisid, curr_refid):
    # insert TF in TargetDBTf table, if TF already does not exist
    db_TF = TargetDBTF.objects.values_list('db_tf_agi', flat=True)
...
def insert_nodes_inplanta(datalist, metadict, curr_metaid, curr_analysisid, curr_refid):
    # insert TF in TargetDBTf table, if TF already does not exist
    db_TF = TargetDBTF.objects.values_list('db_tf_agi', flat=True)
