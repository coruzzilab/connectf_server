def main(TFquery, edges, metadata, targetgenes, output):
...
# @profile
def queryTF(q_tf_list, TFname, edges, edgelist):
...
#@profile
def create_tabular(outfile, rs_final_res, targetgenes, targets_mullist_dict):
    ...
    new_res_df.columns = pd.MultiIndex.from_tuples([('_'.join(c.split('_')[:3]),'_'.join(c.split('_')[3:]))
                                                    for c in new_res_df.columns])

    new_res_df,total_no_exp = self.include_targetcount(new_res_df) # include target count column

    new_res_df.rename(columns={'Full Name__': 'Full Name', 'Name__': 'Name', 'ID__': 'ID', 'pvalue__':
        'pvalue', 'Family__': 'Family', 'Type__': 'Type', 'List__': 'List', 'UserList__':'UserList'}, inplace=True)
    ...
    multi_cols = [('Full Name', 'Gene Full Name'), ('Family', 'Gene Family'),
                    ...
                    ('ID', 'Gene ID')] + multi_cols[-1:] + list_mid_sorted  # rearranging the columns
...
def include_targetcount(new_res_df):
    ...
    tmp_level_sum.drop(['Full Name__', 'Name__', 'ID__', 'Type__', 'Family__', 'pvalue__', 'List__', 'UserList__'],
