...
class Command(BaseCommand):
    ...
    def main(self, pickledir):
        ...
        tgdb_df, anno_df, db_tf, outdirpath = self.read_pickled_json(pickledir) # Func. to write
        self.create_json(tgdb_df, anno_df, db_tf, outdirpath)


    ###################################################################
    # Read pickled json file
    def read_pickled_json(self, pickledir):
    ...
    def create_json(self, query_res_df, mid_tfname, db_tf, output):
        ...
        edges_gm_rawdata, out_tf_genome, tf_genome_matrix_concat = self.get_json_root_edges_genome\
        ...
        self.create_json_object(geneid_genome, edges_gm_rawdata, out_tf_genome, mid_tfname, output_x)
        ...
        edges_tg_rawdata, out_tg_genome, tf_tg_matrix_concat= self.get_json_root_edges_dbase_TGs(query_res_df)
        ...
        self.create_json_object(geneid_TF_targets, edges_tg_rawdata, out_tg_genome, mid_tfname, output_x)
        ...
        edges_db, out_tf_dbase, tf_subset_matrix_final= self.get_json_root_edges_dbase(query_res_df, db_tf)
        geneid_dbase = list(set(tf_subset_matrix_final.columns.tolist()+tf_subset_matrix_final.index.tolist()))
        db_mid_tfname= dict((k, mid_tfname[k]) for k in geneid_dbase if k in mid_tfname) # get a subset from full Ath annotation
        self.create_json_object(geneid_dbase, edges_db, out_tf_dbase, db_mid_tfname, output_view1)
        self.create_json_object(geneid_dbase, edges_db, out_tf_genome, db_mid_tfname, output_view2)
        self.create_json_object(geneid_dbase, edges_db, out_tg_genome, db_mid_tfname, output_view3)


    ###################################################################
    # unstack the df to create edges links
    def create_edge_links(self, rs_final_tfsubset_copy):
    ...
    def get_json_root_edges_dbase(self, query_res_df, db_tf):
        ...
        tf_subset_matrix.rename(columns=lambda x: x.split('_')[0], inplace=True) # rename the df with TF name instead of expID
        ...
        tf_subset_matrix_final.where(tf_subset_matrix_final == 0, 1, inplace=True) # Where keeps if is true and replaces false ones
        # Get edges data
        rs_final_tfsubset.rename(columns=lambda x: x.split('_')[0], inplace=True) # rename the df with TFname instead of expID
        # unstacking the df to create edges links
        stacked_tmp_subset_df = self.create_edge_links(rs_final_tfsubset)
    ...
    def get_json_root_edges_genome(self, rs_final_tfcol_subset, mid_tfname):

        list_tf_genome = [k for k in mid_tfname if 'TXNFACTOR' in mid_tfname[k]] # get a subset from full Ath annotation
        ...
        # unstacking the df to create edges links
        stacked_tmp_subset_df_gm = self.create_edge_links(rs_tfgenome_sub_copy)
    ...
    def get_json_root_edges_dbase_TGs(self, rs_final_tfcol_subset):
        ...
        # unstacking the df to create edges links
        stacked_tmp_subset_df_tg = self.create_edge_links(rs_final_tfcol_copy)
    ...
    # create a json oject
    #@profile
    def create_json_object(self, geneid_x, edges_rawdata, out_tf_x, db_mid_tfname, output_x):
