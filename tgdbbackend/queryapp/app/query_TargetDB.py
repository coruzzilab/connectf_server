...
'''
This script can be used to query the TargetDB database. Retrieves target
genes for transcription factors (TFs) in query list.
Target genes for TFs can be filtered based on edges given in query.
This script can also be used to generate a static table for all the TFs in
the TargetDB (with -e alldata option).
...
Version notes:
Current version of TargetDB database contains data for six transcription
factors:
						TGA1, NLP7, bZIP1 (WT), bZIP1 (mutant), DREB, GATA17,
						HSFB2A
...
def queryTF(sess, q_tf_list, TFname, edges, edgelist, metalist, metadata):
    ...
    for q_tf in q_tf_list:  # fetch data for each TF in a separate DF.
        # Combine all the TF dataframe after this loop.
        ...
        # Allrnaseq or ALLCHIPSEQ will fetch edges present for each TF as
        # opposed to looking for all dbase edges for each TF.
        all_edge_flag = 0
        if edges:
            tmp_edge = ' '.join(edges)
            if 'ALL' in tmp_edge.upper():
                ...
                for pos, val_q_ed in enumerate(edges):
                    ...
                    if 'ALLRNASEQ' in val_q_ed.upper():  # Get ALLRNASEQ
                        ...
                        allquery = '%RNASEQ%'
                        edges = create_all_query(sess, edges, allquery, q_tf,
                                                 pos)
                ...
                for p in sorted(discard_pos,
                                reverse=True):  # this will fail in case when
        ...
        # if df for a Tf is empty after querying the database, don't query
        # the DF for edges or combine to the df for multiple TFs
        if not tf_data.empty:  # if tf_data df is empty, don't append to the
            # tf_frame
            ...
            rs_gp = pd.DataFrame(
                grouped.EDGE.apply(lambda x: ','.join(x)).unstack('META_ID'))

            if edges:  # create query string for pandas.dataframe.query()
                # function
                ...
                if set(edgelist) != set(
                    ...
                    # make a TMP column in DF to tackle if an edges asked is
                    # not present in dbase for a TF
                    rs_gp[
                        'TMP'] = None  # all rows for this column are NONE.
                    # Edge not present for a TF will look for values in this
                    # column and get false as a result of expression

                edgequery = create_pd_query(edges, edgelist, edge_mid_map)

                if edgequery == 'False':  # If no edges are present for 'all'
                    # edges. Ignore these TFs
                    if all_edge_flag != 0:  # if allrnaseq or allchipseq data
                        #  is asked. replace the changed edges (with actual
                        # edges name) to original edges for next TF
                        edges = orig_edges
                        all_edge_flag = 0
                    continue

                rs_gp.query(edgequery,
                            inplace=True)  # query the df of each TF for edges
            if 'TMP' in rs_gp.columns:  # discard the tmp column from the
                # dataframe after querying the DF
                rs_gp.drop('TMP', 1, inplace=True)
            if not rs_gp.empty:  # if dataframe for a TF is not empty after
    ...
    if tf_frames:
        rs_pd_all = pd.concat(tf_frames, axis=1,
                              join='outer')  # concatenate the dfs for
        # multiple TFs: join= 'outer' represents union of multiple dfs
        # query df (df with multiple TFs) if intersection is asked for TFs
        # , otherwise skip as join 'outer' used above creates union between
        # multiple TFs
        # print '***tetsing here= ',''.join(TFname)
        if 'AND' in ''.join(TFname):
            filtered_columns = rs_pd_all.columns.tolist()  # after edges were
            #  removed dataframe contains only valid edges
            tfquery = create_tf_query(TFname, q_tf_list, tf_mid_map,
                                      filtered_columns)
            # print 'tfquery= ',tfquery
            rs_pd_all.query(tfquery,
                            inplace=True)  # query the dataframe for
            # intersection and complex query expression
    else:  # if no data is fetched for the given query then raise an
        # exception and exit
...
def create_pd_query(edges, edgelist, edge_mid_map):
    edgestr = ' '.join(edges)
    edges_in_mid = edgestr.upper().replace(' AND ', ' & ').replace(' OR ',
                                                                   ' | '
                                                                   '').replace(
        ' ANDNOT ', ' &~ '). \
    ...
    for val in edgelist:
        ...
        if not len(edge_mid_map[val]) == 0:
            for i in edge_mid_map[
                val]:  # This code can also handle edges with multiple
                # experiments- creating OR between multiple edges- TESTING
                # REQUIRED HERE
                if count == 0:
        ...
        edges_in_mid = re.sub(r'\b' + val + r'\b', each_edge_mid,
                              edges_in_mid)  # replace value in query
        # expression with pandas query expressions

    return edges_in_mid


##########################################################
# Function to handle unecessary space given in the query- returns a list of
# elements (TF or edges)
def getquerylist(
    query):  # should be able to handle any user entered list: TFs or edges
...
def filter_meta(sess, q_meta, user_q_meta):
    ...
    for valm in q_meta:  # This loop is to simply get the metaids from the
        ...
        rs_meta = sess.query(Meta.meta_id). \
            ...
            filter(Meta.meta_value == valm.split('=')[
            1])  # filtering the metadata and type given in the user query
        valm_format = '"%s"' % (valm.split('=')[1]) + ' in ' + valm.split('=')[
            0]  # create query for meta_data
        user_q_meta_format = user_q_meta_format.replace(valm,
                                                        valm_format)  #
    ...
    db_metadict = dict()
    for valm1 in set(
        rs_meta_tmp):  # This loop is to make combinations on the metaids
        # identified in upper loop
...
def querydb_exp_count(sess, rs_final_res_cols):
    ...
    for id_val in rs_final_res_cols:
        ...
        rs_count = sess.query(func.count(distinct(no2.node_name))). \
            filter(Interactions.meta_id == exp_id).filter(
            Interactions.analysis_id == analys_id). \
            filter(no1.node_id == Interactions.node_1_id).filter(
            Interactions.node_2_id == no2.node_id). \
...
def create_tabular(sess, outfile, rs_final_res, targetgenes, chipdata_summary):
    ...
    rs_final_res.replace('', np.nan,
                         inplace=True)  # without this replacement it was
    # counting '' as an element
    count_series = rs_final_res.count(axis=0)
    # get the tf name in a dict
    mid_tfname_dict = dict()  # dict will contain metaid to genename
    # mapping+TF target counts
    ...
    for i_mid in rs_final_res.columns:  # creating data for excel sheet headers
        tf_name = sess.query(Meta.meta_value).filter(
            Meta.meta_id == ('_'.join(i_mid.split('_')[0:3]))).filter(
            Meta.meta_type == 'TRANSCRIPTION_FACTOR_NAME').all()
        tf_id = sess.query(Meta.meta_value).filter(
            Meta.meta_id == ('_'.join(i_mid.split('_')[0:3]))).filter(
            Meta.meta_type == 'TRANSCRIPTION_FACTOR_ID').all()
        tf_exp = sess.query(Meta.meta_value).filter(
            Meta.meta_id == ('_'.join(i_mid.split('_')[0:3]))).filter(
            Meta.meta_type == 'EXPERIMENT').all()
        tf_control = sess.query(Meta.meta_value).filter(
            Meta.meta_id == ('_'.join(i_mid.split('_')[0:3]))).filter(
            Meta.meta_type == 'CONTROL').all()
        tf_geno = sess.query(Meta.meta_value).filter(
            Meta.meta_id == ('_'.join(i_mid.split('_')[0:3]))).filter(
            Meta.meta_type == 'GENOTYPE').all()
        mid_tfname_dict[i_mid] = str(tf_name[0][0]) + '- ' + str(
            count_series[i_mid]) + ' (' + str(exp_count[i_mid]) + ')'
        tmp_mid_counts[i_mid] = count_series[i_mid]
        mid_tfname[tf_id[0][0]] = str(tf_name[0][0])
        mid_genotype_control[i_mid] = str(tf_exp[0][0]) + ' | ' + str(
            tf_geno[0][0]) + ' | ' + str(tf_control[0][
                                             0])  # include the data for
        # control here in '+' with genotype
    ...
    mid_geno_cntrl_df = pd.DataFrame(data=mid_genotype_control, index=[
        ' '])  # dump mid_genotype_control to a df

    tmp_chip_coding = pd.DataFrame(data=chipdata_summary,
                                   index=[' '])  # dump chipdata_summary to a df
    tmp_rnaseq_df = pd.DataFrame(data=tmp_rnaseq_summary,
                                 index=[' '])  # dump tmp_rnaseq_summary to a df

    tmp_chip_coding.rename(columns=lambda x: x[::-1].replace('_', '.', 1)[::-1],
                           inplace=True)  # replacing the last occurence of
    # '_' with '.'
    ...
    for k in all_targetgenes:
        rs_gnames = sess.query(Genenames.ath_id, Genenames.ath_name,
                               Genenames.ath_fullname, Genenames.ath_gene_type,
                               Genenames.ath_gene_fam). \
            filter(Genenames.ath_id == k.strip()).all()
        rsall_gnames.extend(rs_gnames)
    df_target_names = pd.DataFrame(rsall_gnames,
                                   columns=['ID___Gene ID', 'Name___Gene Name',
                                            'Full Name___Gene Full Name',
                                            'Type___Gene Type',
                                            'Family___Gene Family']).set_index(
        'ID___Gene ID')
    ...
    new_res_df.columns = pd.MultiIndex.from_tuples([('_'.join(c.split('_')[
                                                    ...
                                                    for c in
                                                    new_res_df.columns])
    # new_res_df.rename(columns=lambda x: pd.MultiIndex.from_tuples([(
    # '_'.join(x.split('_')[:3]), '_'.join(x.split('_')[3:]))]),
    # inplace=True) # probably a better way of renaming but not working
    ...
    total_no_exp = '(' + str(
        len(list(set(tmp_level_sum.columns.get_level_values(0))))) + ')'
    new_res_df['Target Count', total_no_exp] = (level_count.notnull() * 1).sum(
        axis=1)
    new_res_df['Target Count', total_no_exp] = new_res_df[
                                                   'Target Count',
                                                   total_no_exp].ix[
                                               3:].astype(np.int64).astype(str)
    new_res_df['Target Count', total_no_exp] = new_res_df[
                                                   'Target Count',
                                                   total_no_exp].ix[
                                               3:].apply(
    ...
    new_res_df.sort([('Target Count', total_no_exp)], ascending=False,
                    inplace=True,
                    na_position='first')  # na_position='first' to leave the
    # header cols (na.nan values) sorted first
    # new_res_df.replace(np.nan,'', regex=True, inplace=True)
    if df_count_rows > 1:  # Writing dataframe to excel and formatting the
        # excel output
        writer = pd.ExcelWriter(outfile + '/' + outfile.split('/')[
            -1] + '_tabular_output.xlsx')  # output in excel format
        new_res_df.to_excel(writer,
                            sheet_name='TargetDB Output')  # THIS IS THE
...
def get_indegree_outdegree(tf_subset_matrix_concat, TF_tofind):
    list_of_tfs = tf_subset_matrix_concat[
        tf_subset_matrix_concat[TF_tofind] > 0].index.tolist()
...
def write_to_excel(writer, new_res_df):
    ...
    header_fmt = workbook.add_format(
        {'font_name': 'Calibri', 'font_size': 15, 'bold': True,
         'align': 'center', 'border': 1})
    header_fmt_1 = workbook.add_format(
        {'font_name': 'Calibri', 'font_size': 11, 'bold': True,
         'align': 'center', 'border': 1})
    worksheet.set_row(2, None, None, {
        'hidden': True})  # hiding unnecessary row created by multiindexing
    worksheet.set_row(3, None, header_fmt_1)
    worksheet.set_row(4, None, header_fmt)
    worksheet.set_row(5, None, header_fmt)
    format1 = workbook.add_format(
        {'bg_color': '#FA8072', 'font_color': '#000000'})
    format2 = workbook.add_format(
        {'bg_color': '#98FB98', 'font_color': '#000000'})
    format3 = workbook.add_format(
        {'bg_color': '#FFFF99', 'font_color': '#000000'})
...
def create_sif(sess, output, tmp_df, targetgenes):
    # before creating the sif file I am combining chipseq data from different
    #  time-points (from one experiment) into one column
    # Now I am retaining only one edge for multiple time-points: e.g.:
    # Target:CHIPSEQ:0,Target:CHIPSEQ:5 will be replaced by Target:CHIPSEQ
    sif_rs_tabular = tmp_df.groupby(tmp_df.columns, axis=1). \
        apply(
        lambda x: x.apply(lambda y: ','.join([l for l in y if l is not None]),
                          axis=1))
    sif_rs_tabular.replace({'^TARGET:CHIPSEQ.*': 'TARGET:CHIPSEQ'}, regex=True,
                           inplace=True)  # replace Target:CHIPSEQ:0,
    # Target:CHIPSEQ:5 with Target:CHIPSEQ
    sif_rs_tabular.replace('', np.nan, inplace=True)

    stacked_tmp_df = pd.DataFrame(
        sif_rs_tabular.stack().reset_index())  # stack converts df columns
    # into stacked rows
    stacked_tmp_df.columns = ['TARGET', 'TF', 'EDGE']  # assign new columns
    stacked_tmp_df['TF'] = stacked_tmp_df['TF'].apply(lambda x: x.split('#')[0])
    # extract experimentID from experimentID+analysisID (separated by '#')
    stacked_tmp_df = stacked_tmp_df.drop_duplicates()  # dropping duplicates
    # to solve the problem of multiple representation of same experiment (
    # with multiple analysis ID) and chipseq multiple time-points
    stacked_tmp_df['TF'] = stacked_tmp_df['TF'].apply(
        lambda x: x.split('_')[0])  # extract TFname from experimentID
    tf_list = list(set(stacked_tmp_df['TF'].tolist()))
    reordered_tmp_df = pd.DataFrame(stacked_tmp_df, columns=['TF', 'EDGE',
                                                             'TARGET'])  #
    ...
    total_exp = len(
        sif_rs_tabular.columns.tolist())  # count the total number of
    # experiments
    sif_rs_tabular['target_count'] = (sif_rs_tabular.notnull() * 1).sum(axis=1)
    sub_common_targets = sif_rs_tabular[sif_rs_tabular[
                                            'target_count'] == total_exp]  #
    # df subset of common targets (i.e. targets repersented across all the exps)
    sub_shared_targets = sif_rs_tabular[sif_rs_tabular[
                                            'target_count'] > 1]  # df subset
    #  of shared targets (i.e. target genes present in >1 exp)
    sub_common_targets.drop('target_count', 1,
                            inplace=True)  # drop the target_count column
    # from this subset df
    sub_shared_targets.drop('target_count', 1,
                            inplace=True)  # drop the target_count column
    # from this subset df
    ...
    stacked_common_targets['TF'] = stacked_common_targets['TF'].apply(
        lambda x: x.split('_')[0])  # extract TFname from experimentID
    stacked_common_targets[['TF', 'EDGE', 'TARGET']].to_csv(outfile_common,
                                                            sep='\t',
                                                            index=False)
    ...
    stacked_shared_targets['TF'] = stacked_shared_targets['TF'].apply(
        lambda x: x.split('_')[0])  # extract TFname from experimentID
    stacked_shared_targets[['TF', 'EDGE', 'TARGET']].to_csv(outfile_shared,
                                                            sep='\t',
                                                            index=False)
...
def getmetadata(sess, df_cols, writer):
    # This function takes a list of metadata ids as an input, returns a nested
    #  dict that contains all the metadata types for metadata ids
    db_metadict = dict()
    expid_dict = dict()
    list_metaid = ['_'.join(x.split('_')[:3]) for x in df_cols]
    # print 'list_metaid= ',list_metaid
    for valm in set(list_metaid):
        rs_meta = sess.query(Meta.meta_id, Meta.meta_value,
                             Meta.meta_type).filter(Meta.meta_id == valm)
    ...
    out_metadata_df = pd.concat([tf_name, metadata_df],
                                axis=0)  # THIS IS MY FINAL OUTPUT DATAFRAME
    # FOR METADATA OUTPUT
    ...
    bold_font1 = workbook1.add_format(
        {'bold': True, 'font_size': 13, 'border': 1, 'align': 'left'})
...
def tabular(rs_final_res_t):
    ...
    rs_tabular = rs_final_res_t.groupby(rs_final_res_t.columns, axis=1). \
        apply(lambda x: x.apply(lambda y: ','.join(
        [':'.join(l.split(':')[2:]) for l in y if l is not None]), axis=1))
    ...
    for c_c in set(chipseq_cols):
        ...
        chipdata_summary[c_c] = ':'.join(str(k) for k in sorted(
            [int(x_c_dict.split(':')[2]) for x_c_dict in
             list(set(all_timepoints))]))

    for col_chip_dict in chipdata_summary:
        binary_code_dict = dict()
        count_ele = len(chipdata_summary[col_chip_dict].split(':'))
        for flag, values in enumerate(
            chipdata_summary[col_chip_dict].split(':')):
            binary_code_dict[values] = 10 ** ((count_ele - flag) - 1)

        rs_tabular[col_chip_dict + '_binary'] = rs_tabular.apply(
...
def main(dbname, TFquery, edges, metadata, output, targetgenes):
    ...
    # engine= create_engine('mysql://coruzzilab:accesstargetdb@172.22.2.137
    # /'+dbname)
    ...
    if 'ALLTF' in tmptf.upper():  # if input query has all TFs
        all_tfs = sess.query(Nodes.node_name, Nodes.node_type).filter(
            Nodes.node_type == 'TF').all()
        for x in all_tfs:
    ...
    rs_final_res = queryTF(sess, q_tf_list, TFname, edges, edgelist,
                           rs_meta_list, metadata)
    if not rs_final_res.empty:
        # if chipseq in dataframe column- exclude the time point from meta_id
        rs_final_res.columns = [
        ...
        shutil.make_archive(output, 'zip',
                            output)  # create a zip file for output directory
        shutil.rmtree(
        ...
        new_res_df = pd.DataFrame.from_dict(message_dict, orient='index',
                                            dtype=None)
        new_res_df.columns = ['1']
        out_metadata_df = pd.DataFrame.from_dict(message_dict, orient='index',
                                                 dtype=None)
...
if __name__ == '__main__':
    ...
    main(args.dbname, args.TFname, args.edges, args.metadata, args.output,
         args.targetgenes)
