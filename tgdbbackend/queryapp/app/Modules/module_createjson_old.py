...
def create_json(sess, query_res_df, output):
    ...
    # function 2 to create the raw data for creating json file for whole
    # genome TFs- Raw data means nodes, edges, size of the nodes
    ...
    # json_object_WG= create_json_object(geneid_genome, edges_json_genome,
    # out_tf_genome, mid_tfname_genome, tfname_tftype, output_x)

    # function 3 to create the raw data for creating json file for
    # TFs-Targets- Raw data means nodes, edges, size of the nodes
    ...
    mid_tfname_dbase, tfname_tftype = id_name_mapping(sess,
                                                      geneid_TF_targets)  #
    # query the database for geneid to genename mapping
    # json_object_TG= create_json_object(geneid_TF_targets, edges_json_tg,
    # out_tg_genome, mid_tfname_dbase, tfname_tftype, output_x)

    # function 1 to create the raw data for creating json file for database
    # TFs- Raw data means nodes, edges, size of the nodes
    ...
    json_object_TFdbase_vw1 = create_json_object(geneid_dbase, edges_json_list,
                                                 out_tf_dbase, mid_tfname_dbase,
                                                 tfname_tftype, output_view1)
    # json_object_TFdbase_vw2= create_json_object(geneid_dbase,
    # edges_json_list, out_tf_genome, mid_tfname_dbase, tfname_tftype,
    # output_view2)
    # json_object_TFdbase_vw3= create_json_object(geneid_dbase,
    # edges_json_list, out_tg_genome, mid_tfname_dbase, tfname_tftype,
    # output_view3)

    # return json_object_WG, json_object_TG, json_object_TFdbase_vw1,
...
def get_json_root_edges_dbase(rs_final_tfcol_subset, df_tf_list):
    ...
    tf_subset_matrix_concat = tf_subset_matrix.groupby(tf_subset_matrix.columns,
                                                       axis=1). \
    ...
    tf_subset_matrix_concat.where(tf_subset_matrix_concat == 0, 1,
    ...
    if uniqueele:
        ...
        for uni_val in uniqueele:
            tmp_tfs_dict = {
                uni_val: [0] * len(tf_subset_matrix_concat.columns.tolist())}
        tmpdf = pd.DataFrame(tmp_tfs_dict,
                             index=tf_subset_matrix_concat.columns.tolist())
        # create a tmp df to include data for non-target dfs
        tmp_frames = [tf_subset_matrix_concat,
                      tmpdf.transpose()]  # transpose the tmpdf as missing
        # tfs are columns in the tmpdf df
        tf_subset_matrix_final = pd.concat(
            tmp_frames)  # concat the dataframes: output+tmpdf (data for
        # non-target dfs)
    else:
        tf_subset_matrix_final = tf_subset_matrix_concat  # This is the final
        #  dataframe that contains TF-TF connections

    # unstacking the df to create edges links
    stacked_root_df = pd.DataFrame(
        tf_subset_matrix_final.unstack().reset_index())
    transposed_df = tf_subset_matrix_final.transpose()  # transpose the df to
    ...
    stacked_tmp_subset_df['id'] = stacked_tmp_subset_df['source'] + '_' + \
                                  stacked_tmp_subset_df[
                                      'target']  # create ID for edges (
    # TFID_TargetID)
    stacked_tmp_subset_df.drop('val', 1,
                               inplace=True)  # we don't need the dataframe
    # values (0 1)
    ...
    for rowdf in (stacked_tmp_subset_df.to_dict(
    ...
    tf_subset_matrix_final_copy['row_sum'] = tf_subset_matrix_final_copy.sum(
        axis=1)
    tf_subset_matrix_final_copy['column_sum'] = \
        tf_subset_matrix_final_copy.sum(axis=0)
    # rootlist= tf_subset_matrix_final_copy[(tf_subset_matrix_final_copy[
    # 'row_sum']==0) & (tf_subset_matrix_final_copy[
    # 'column_sum']>0)].index.tolist()
...
def get_json_root_edges_genome(sess, rs_final_tfcol_subset, df_rows):
    ...
    # unstacking the df to create edges links
    stacked_root_df_genome = pd.DataFrame(
        tf_genome_matrix_concat.unstack().reset_index())
    transposed_df = tf_genome_matrix_concat.transpose()  # transpose the df
    # to get the outdegree- because in original df outdegree is columns
    stacked_tmp_subset_df_genome = pd.DataFrame(transposed_df[
                                                    transposed_df > 0].stack(

    ).reset_index())  # make the sif like format: exclude where tfs has no
    # connection (0 in df)
    stacked_tmp_subset_df_genome.columns = ['source', 'target',
                                            'val']  # assign columns to the df
    stacked_tmp_subset_df_genome['id'] = stacked_tmp_subset_df_genome[
                                             'source'] + '_' + \
                                         stacked_tmp_subset_df_genome[
                                             'target']  # create ID for edges
    #  (TFID_TargetID)
    stacked_tmp_subset_df_genome.drop('val', 1,
                                      inplace=True)  # we don't need the
    # dataframe values (0 1)

    edges_json_genome = list()  # object to be included in json final dict-
    # that will be dupmed to a json file
    for rowdf in (stacked_tmp_subset_df_genome.to_dict(
        orient='records')):  # converting df to a dict to create json format
        # list
...
def get_json_root_edges_dbase_TGs(sess, rs_final_tfcol_subset, df_tf_list):
    ...
    tf_tg_matrix_concat = tf_tg_matrix.groupby(tf_tg_matrix.columns, axis=1). \
        apply(lambda p: p.apply(lambda q: 1 if
    sum(q.values) > 0 else 0,
                                axis=1))  # merge cols for tfs (1 for
    # interaction with row TF else 0)
    ...
    # unstacking the df to create edges links
    stacked_root_df_tg = pd.DataFrame(
        tf_tg_matrix_concat.unstack().reset_index())
    transposed_df_tg = tf_tg_matrix_concat.transpose()  # transpose the df to
    #  get the outdegree- because in original df outdegree is columns
    stacked_tmp_subset_df_tg = pd.DataFrame(transposed_df_tg[
                                                transposed_df_tg > 0].stack(

    ).reset_index())  # make the sif like format: exclude where tfs has no
    # connection (0 in df)
    stacked_tmp_subset_df_tg.columns = ['source', 'target',
                                        'val']  # assign columns to the df
    stacked_tmp_subset_df_tg['id'] = stacked_tmp_subset_df_tg['source'] + '_' \
                                     + \
                                     stacked_tmp_subset_df_tg[
                                         'target']  # create ID for edges (
    # TFID_TargetID)
    stacked_tmp_subset_df_tg.drop('val', 1,
                                  inplace=True)  # we don't need the
    # dataframe values (0 1)
...
###################################################################
# create a json oject
def create_json_object(geneid_x, edges_json_list, out_tf_x, mid_tfname,
    ...
    for col_tfs in geneid_x:
        ...
        if col_tfs in tfname_tftype:
            if tfname_tftype[col_tfs] == 'TXNFACTOR':
                ...
                tmp_tfdict['data']['type'] = tfname_tftype[col_tfs]  # gene type
                tmp_tfdict['data'][
                    'color'] = "#33C1FF"  # Assign color to non-TFs
                tmp_tfdict['data'][
                    'shape'] = "ellipse"  # Assign shape to non-TFs
