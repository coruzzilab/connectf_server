...
def create_json(sess, query_res_df, output):
	...
	# function 3 to create the raw data for creating json file for whole genome TFs- Raw data means nodes, edges, size of the nodes, edge weight
	output_x= output+'_genome'
	edges_gm_rawdata, out_tf_genome, tf_genome_matrix_concat= get_json_root_edges_genome(sess, query_res_df, df_rows)
	geneid_genome=  list(set(tf_genome_matrix_concat.columns.tolist()+tf_genome_matrix_concat.index.tolist())) # For database TF matrix I need columns and rows (genome TF) as nodes
	mid_tfname_genome, tfname_tftype= id_name_mapping(sess, geneid_genome) # query the database for geneid to genename mapping
	#json_object_WG= create_json_object(geneid_genome, edges_gm_rawdata, out_tf_genome, mid_tfname_genome, tfname_tftype, output_x)

	# function 4 to create the raw data for creating json file for TFs-Targets- Raw data means nodes, edges, size of the nodes, edge weight
	output_x= output+'_targets'
	edges_tg_rawdata, out_tg_genome, tf_tg_matrix_concat= get_json_root_edges_dbase_TGs(query_res_df, df_rows)
	...
	#json_object_TG= create_json_object(geneid_TF_targets, edges_tg_rawdata, out_tg_genome, mid_tfname_dbase, tfname_tftype, output_x)
	
	# function 2 to create the raw data for creating json file for database TFs- Raw data means nodes, edges, size of the nodes, edge weight
	#output_view1= output+'_dbase_view1'
	output_view1= output
	output_view2= output+'_dbase_view2'
	output_view3= output+'_dbase_view3'
	edge_rawdata, out_tf_dbase, tf_subset_matrix_final= get_json_root_edges_dbase(query_res_df, df_tf_list)
	...
	json_object_TFdbase_vw1= create_json_object(geneid_dbase, edge_rawdata, out_tf_dbase, mid_tfname_dbase, tfname_tftype, output_view1)
	#json_object_TFdbase_vw2= create_json_object(geneid_dbase, edge_rawdata, out_tf_genome, mid_tfname_dbase, tfname_tftype, output_view2)
	#json_object_TFdbase_vw3= create_json_object(geneid_dbase, edge_rawdata, out_tg_genome, mid_tfname_dbase, tfname_tftype, output_view3)

	# function 1 to create the raw data for creating json file for query TFs- Raw data means nodes, edges, size of the nodes, edge weight
	output_query= output+'_query'
	edge_query_rawdata, out_query_dbase, tf_query_matrix_final= get_json_root_edges_query(query_res_df, df_tf_list)
	geneid_query= list(set(tf_query_matrix_final.columns.tolist()+tf_query_matrix_final.index.tolist()))
	mid_tfname_query, tfname_tftype= id_name_mapping(sess, geneid_query) # query the database for geneid to genename mapping
	#json_object_query= create_json_object(geneid_query, edge_query_rawdata, out_query_dbase, mid_tfname_query, tfname_tftype, output_query)

	#return json_object_WG, json_object_TG, json_object_query, json_object_TFdbase_vw1, json_object_TFdbase_vw2, json_object_TFdbase_vw3
...
def create_edge_links(rs_final_tfsubset_copy):
	...
	transposed_df= rs_final_tfsubset_copy.transpose() # transpose the df to get the outdegree- because in original df outdegree is columns
	...
	stacked_tmp_subset_df['id']= stacked_tmp_subset_df['source']+'_'+stacked_tmp_subset_df['target']+'_'+stacked_tmp_subset_df['val'] # create ID for edges (TFID_TargetID)
	stacked_tmp_subset_df['color_induced']= stacked_tmp_subset_df['val'].apply(lambda x: '#669900' if 'INDUCED' in x else '')
	stacked_tmp_subset_df['color_repressed']= stacked_tmp_subset_df['val'].apply(lambda x: '#FF6633' if 'REPRESSED' in x else '')
	stacked_tmp_subset_df['color_chipseq']= stacked_tmp_subset_df['val'].apply(lambda x: '#666666' if x.isdigit() else '')
	stacked_tmp_subset_df['color'] = stacked_tmp_subset_df['color_chipseq'].map(str)+stacked_tmp_subset_df['color_induced'].map(str)+stacked_tmp_subset_df['color_repressed'].map(str)
...
def get_json_root_edges_query(rs_final_tfcol_subset, df_tf_list):
	...
	cols_tfs_list= list(set([x.split('_')[0] for x in df_columns])) # from df columns create a 

	rs_final_tfquery= rs_final_tfcol_subset[rs_final_tfcol_subset.index.isin(cols_tfs_list)] # get rows with TF queried by user
	tf_query_matrix= rs_final_tfquery.notnull() * 1 # convert edges (values in df) into binary format
	tf_query_matrix.rename(columns=lambda x: x.split('_')[0], inplace=True) # rename the dataframe with TF name instead of experimentID
	tf_query_matrix_final= tf_query_matrix.groupby(tf_query_matrix.columns, axis=1).\
	...
	tf_query_matrix_final.where(tf_query_matrix_final==0,1,inplace=True) # Where command keeps values where condition is true and replaces false ones
	
	# Get edges data
	rs_final_tfquery_copy= rs_final_tfquery.copy(deep=True)
	rs_final_tfquery_copy.rename(columns=lambda x: x.split('_')[0], inplace=True) # rename the dataframe with TF name instead of experimentID
	...
	tf_query_matrix_final_copy= tf_query_matrix_final.copy(deep=True) # creating a deep copy of the df to avoid changing the df
	
	for stsd in tf_query_matrix_final_copy:
		tf_query_matrix_final_copy.loc[stsd, stsd] = 0 # replace values for TF-self connections to zero
	
	out_querytf_dbase= (tf_query_matrix_final_copy*1).sum(axis=0) # store this data in a dict
	tf_query_matrix_final_copy['row_sum']= tf_query_matrix_final_copy.sum(axis=1)
	tf_query_matrix_final_copy['column_sum']= tf_query_matrix_final_copy.sum(axis=0)
...
def get_json_root_edges_dbase(rs_final_tfcol_subset, df_tf_list):
	...
	# Replacing here all the > 0 values with 1 to count no. of genes each TF is targeting
	tf_subset_matrix_final.where(tf_subset_matrix_final==0,1,inplace=True) # Where command keeps values where condition is true and replaces false ones
	
	# Get edges data
	rs_final_tfsubset_copy= rs_final_tfsubset.copy(deep=True)
	rs_final_tfsubset_copy.rename(columns=lambda x: x.split('_')[0], inplace=True) # rename the dataframe with TF name instead of experimentID
	# unstacking the df to create edges links
...
def get_json_root_edges_genome(sess, rs_final_tfcol_subset, df_rows):
	...
	rs_tfgenome_sub_copy.rename(columns=lambda x: x.split('_')[0], inplace=True)# rename the dataframe with TF name instead of experimentID
	# unstacking the df to create edges links
...
def get_json_root_edges_dbase_TGs(rs_final_tfcol_subset, df_tf_list):
	...
	rs_final_tfcol_copy.rename(columns=lambda x: x.split('_')[0], inplace=True)# rename the dataframe with TF name instead of experimentIDs
	# unstacking the df to create edges links
...
###################################################################
# create a json oject
def create_json_object(geneid_x, edges_rawdata, out_tf_x, mid_tfname, tfname_tftype, output_x):
	...
	fontsize_edges= (np.log2(out_tf_x.max()+2))*10 # font size edges= size of TF with maximum no. of targets- normalize on log2 scale
	
	# Assigning weigths to the edges- this is only for chip-seq data
	edges_rawdata_copy['weight']= edges_rawdata_copy['val'].apply(lambda x: ((x.count("1"))*fontsize_edges) if x.isdigit() else fontsize_edges)
	edges_rawdata_copy.drop(['val','color_induced','color_repressed','color_chipseq'], 1, inplace=True) # we don't need the dataframe values (0 1)
	
	edges_json_list= list() # object to be included in json final dict- that will be dupmed to a json file
	for rowdf in (edges_rawdata_copy.to_dict(orient='records')): # converting df to a dict to create json format list
	...
	for col_tfs in geneid_x:
		...
		tmp_tfdict['data']['weight']= ((np.log2(fontsize+2))*100)/3 # for font size 1/3 of size of TF with maximum no. of targets
