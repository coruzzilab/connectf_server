...
class Meta(Base):
    __tablename__ = 'Meta'
    meta_p = Column(Integer, primary_key=True,
                    autoincrement=True)  # I had to make this a primary key
    meta_id = Column(String(100), primary_key=True, nullable=False, index=True)
    analysis_id = Column(String(100), primary_key=True, nullable=False,
                         index=True)
...
class Interactions(Base):
    ...
    meta_id = Column(String(100), ForeignKey('Meta.meta_id'),
                     nullable=False)  # could not change this to an interger
    # because of autoincrement
    analysis_id = Column(String(100), ForeignKey('Meta.analysis_id'),
                         nullable=False)
...
class createdbase():
    def __init__(self, dbasename):
        ...
        self.engine.execute(
            "DROP DATABASE IF EXISTS " + dbasename)  # uncomment this if you
...
class AccessDatabase():  # This class has special permissions for coruzzilab
    ...
    def __init__(self, dbname):
        self.engine = create_engine(
            'mysql://coruzzilab:accesstargetdb@172.22.2.137/' + dbname)
